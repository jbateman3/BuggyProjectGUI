...
var test = new glUtil();
    ...
    Model.prototype._compileMaterials = function (gl, meshes) {
        ...
        for (i in meshes) {
            mesh = meshes[i];
            mesh.diffuse = test.loadTexture(gl, mesh.defaultTexture);
        }
    };
    ...
    Model.prototype.draw = function (gl, viewMat, projectionMat) {
		...
		var copy1 = mat4.create();
        mat4.set(viewMat, copy1);
		var copy2 = mat4.create();
        mat4.set(projectionMat, copy2);
		
        gl.uniformMatrix4fv(shader.uniform.viewMat, false, copy1);
        gl.uniformMatrix4fv(shader.uniform.modelMat, false, identityMat);
        gl.uniformMatrix4fv(shader.uniform.projectionMat, false, copy2);
